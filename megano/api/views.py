...
from products.models import Product, Category, SubCategory, ProductReviews, ProductSpecifications, Tag, ProductSale
			...
			"subcategories": [
				{
					...
					"image": {
						"src": "http://127.0.0.1:8000/media/{}".format(subcategory.preview),
						"alt": "Image alt string"
					}
...
def catalog(request):
	...
	products = Product.objects.filter(name__icontains=filter_catalog['filter[name]']) & Product.objects.filter(
		price__range=(filter_catalog['filter[minPrice]'], filter_catalog['filter[maxPrice]']))
...
def sales(request):
	discounted_products = ProductSale.objects.filter(date_to__gte=datetime.datetime.now()).order_by('date_to')
		...
		'items': [
			{
				...
				"images": [
					{
						"src": "http://127.0.0.1:8000/media/{}".format(sale.product.preview),
						"alt": "hello alt",
					}
				],
			...
			cart_items.append(
				{
					...
					"images": [
						{
							"src": "http://127.0.0.1:8000/media/{}".format(product.preview),
							"alt": "hello alt",
						}
					],
					"tags": [
				...
				"products": [
					{
						...
						"images": [
							{
								"src": "http://127.0.0.1:8000/media/{}".format(product.product.preview),
								"alt": "Image alt string"
							}
						],
						"tags": [
...
def tags(request):
	...
	print(tag_list)
	data = [
		{"id": tag.pk, "name": tag.name} for tag in tag_list
	]
	return JsonResponse(data, safe=False)
	...
	if (request.method == 'GET'):
		...
		for product in order_products:
			if not product.product.free_delivery:
				order.total_cost += 5
				order.delivery_type += '\n+ 5 for paid delivery of {product}'.format(product=product.product.name)
